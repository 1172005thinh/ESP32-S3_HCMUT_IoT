...
window.addEventListener('load', () => {
    initWebSocket();
    // Delay renderGauges để đảm bảo thư viện JustGage đã load
    setTimeout(() => {
        renderGauges();
    }, 500);
});
...
function toggleSidebar() {
    ...
    if (sidebar) {
        ...
        // Thay đổi logo khi sidebar collapse/expand
        if (sidebar.classList.contains('collapsed')) {
            logoImg.src = 'logo_org1.png'; // Logo nhỏ khi collapse
        } else {
            logoImg.src = 'logo_org.png'; // Logo lớn khi expand
        }
    }
}
...
function renderGauges() {
    ...
    // Xóa nội dung cũ trước khi vẽ mới để tránh bị chồng đè
